...
int main (int argc, char * argv[])
{
   ...
   ValueType *a = aligned_alloc<double>(nmax+5);
   ValueType *b = aligned_alloc<double>(nmax+5);

//#if defined(__INTEL_COMPILER) && defined(__ALIGNMENT)
//#warning 'Assuming alignment'
      ...
      for (int k = 0; k < niters; ++k)
      {
         ...
         // FILL (a[] := s): set to scalar value.
         {
            ...
            for (int loop = 0; loop < nloop; ++loop)
            {
               ...
               #pragma omp simd aligned(a : Alignment())
               for (int i = 0; i < m; ++i)
                  a[i] = s;
            }
