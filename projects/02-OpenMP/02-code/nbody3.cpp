...
/*
 * ON MAC NEED TO CHANGE THE @rpath/libiomp5.dylib
 * FILE TO THE LASTEST PATH USING THE FOLLOWING COMMAND
 *
 * install_name_tool -change @rpath/libiomp5.dylib \
 * /opt/intel/compilers_and_libraries_2018.0.104/mac/compiler/lib/libiomp5.dylib \
 * nbody3
 *
 * */
...
# endif
   ...
   else
   {
	...
	/**
	for (int k = 0; k < NDIM; ++k){
		post_array(i,k) = 2 * (frand<ValueType>() - 0.5)
		vel_array(i,k) = 0;
		acc_array(i,k) = 0;
	}
